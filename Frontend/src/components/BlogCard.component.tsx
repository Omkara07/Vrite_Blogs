...
const BlogCard: React.FC<blogs> = ({ blog: { blog_id, title, des, banner, activity, publishedAt, tags }, author: { fullname, profile_img, username } }) => {
    return (
        <Link to={`/blog/${blog_id}`} className='pl-6 md:pl-0 flex w-full items-center gap-8 justify-between p-4 border-b'>
            <div className='flex flex-col gap-2 text-sm'>
                ...
                <p className='text-gray-600 font-semibold font-gelasio leading-7 max-sm:hidden md:max-[1100px]:hidden line-clamp-1'>{des}</p>
                <div className='flex gap-4 items-center mt-4'>
                    <p className='text-black max-w-fit text-sm items-center flex px-3 py-1 bg-gray-300 font-semibold rounded-xl'>{tags[0]}</p>
